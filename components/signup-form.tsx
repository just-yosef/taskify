...
export function SignupForm({ switchMode }: { switchMode: () => void }) {
  ...
  const { confirmPassword, email, fullName, password, } = formState.errors
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <FieldGroup className="gap-3">
            <Field>
              ...
              <Input id="name" {...register("fullName", { required: true })} type="text" />
            ...
            <FieldGroup>
              <Field>
                <Button type="submit" disabled={isPending} variant="teal">Create Account</Button>
                <FieldDescription className="px-6 text-center">
                  Already have an account?
                  <span className="underline cursor-pointer" onClick={switchMode}>
                    Sign in
                  </span>
                </FieldDescription>
              </Field>
